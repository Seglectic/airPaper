			...
			function createWO(paperData){
					paperData.order_items.forEach(orderItem => {   		// Iterate through all actual parts in the order
					...
					let comp = orderItem.components[0];				// Grab the first 'Component' for the order (orders can be assemblies that get messy)
					...
					// ╭───────────────────────────────────────────────────╮
					// │  Send to Airtable if not export controlled (ITAR) │
					// ╰───────────────────────────────────────────────────╯
					let ITAR = orderItem.export_controlled // bool
					if (!ITAR){																																							  // Grab the pic and send it if it's not ITAR-y
						if (thumbURL){																																		    	// If the component in the order has an attached pic
					...
					}else{ 																																			 							// If we've got an ITAR component
